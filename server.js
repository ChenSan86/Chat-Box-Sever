...
const logger = winston.createLogger({//winston日志库
  transports: [
...
const redis = new Redis({
  ...
  enableOfflineQueue: true, // 启用离线队列缓冲。启用离线队列，将未完成的命令缓存起来，等待重新连接后执行。
  maxRetriesPerRequest: 5, // 增加重试次数，每个请求的最大重试次数。
  retryDelay: 3000,
});
